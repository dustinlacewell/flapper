import { AllFiles } from "./src/procs/AllFiles";
import { BindMDX } from "./src/procs/BindMDX";
import { CreatePages } from "./src/procs/CreatePages";
import { SetRelativePath } from './src/procs/SetRelativePath';
import { SetURL } from "./src/procs/SetURL";
...
const writing = [
    {
        type: 'page',
        pipeline: [
            CreatePages('src/templates/MdxPage/index.tsx'),
        ],
    },
]
...
export const createPages = async (createPageUtils: CreatePageUtils) => {
    const context = new Context(createPageUtils);

    for (const stage of stages) {
        const { type, pipeline } = stage;
        const assets = context.getAssetType(type)

        for (const proc of pipeline) {
            await proc(context, type, assets)
        }
